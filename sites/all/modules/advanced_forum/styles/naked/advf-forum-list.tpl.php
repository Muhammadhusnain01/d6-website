<?php
// $Id: advf-forum-list.tpl.php,v 1.1.2.21 2009/01/27 23:31:11 michellec Exp $
                    ...
                    <?php if ($forum->description): ?>
                      <div class="description">
                        ...
                        <div class="campaigns">
                          <?php if ($child_id == 8) { ?>
                            <?php $campaigns = ''; ?>
                            <?php foreach ($forums as $children_child_id => $children_forum): ?>
                              <?php if (in_array(8, $children_forum->parents)){ ?>
                                <?php $campaigns .= '<a href="'. $children_forum->link .'">'. $children_forum->name .'</a>'; ?>
                                <?php if ($children_forum->new_posts > 0){ ?>
                                  <?php if ($children_forum->new_posts == 1): ?>
                                    <?php $campaigns .= ' (<a href="'. $children_forum->new_url_posts .'">'. t('1 new') .'</a>)'; ?>
                                  <?php else: ?>
                                    <?php $campaigns .= ' (<a href="'. $children_forum->new_url_posts .'">'. t('@count new', array('@count' => $children_forum->new_posts)) .'</a>)'; ?>
                                  <?php endif; ?>
                                <?php } ?>
                                <?php $campaigns .= ' | '; ?>
                              <?php }?>
                            <?php endforeach; ?>
                            <?php $campaigns = substr($campaigns, 0, -3); ?>
                            <?php print $campaigns; ?>
                          <?php } ?>
                        </div>
                      </div>
